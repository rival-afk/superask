...
Использование:
  ...
  sa apikey <ключ>              — Установить API-ключ opencode/zen
  ...
  sa wol on/off                — Включить/выключить Wake-on-LAN
  sa proxy <url>                — Установить прокси для Telegram API
  sa proxy off                  — Отключить прокси
  sa server <url>               — Установить URL Render-сервера
  sa apikey <ключ>              — Установить API-ключ opencode/zen
  sa mode                       — Показать режим (proxy/relay)
  sa mode proxy                 — Бот на ПК, Render как прокси (по умолчанию)
  sa mode relay                 — Бот на Render, агент опрашивает
  sa agent on/off/restart/status/logs — Управление агентом
  sa help                       — Эта справка
...
def cmd_status():
    ...
    if proxy_url:
        print(f"  Прокси:             ✅ {proxy_url[:40]}...")
    else:
        print(f"  Прокси:             ❌ не используется")

    server_url = config.get_server_url()
    if server_url:
        print(f"  Render сервер:      {server_url}")

    print(f"  Режим:              {config.get_agent_mode()}")

    api_key = config.get_api_key()
    if api_key:
        print(f"  API-ключ:           ✅ {api_key[:12]}...")
    else:
        print(f"  API-ключ:           ❌ не задан")
        _e("  Получить: https://opencode.ai/zen")
...
def _wol_on():
    if not shutil.which("ethtool"):
        _die("ethtool не установлен. Установите: sudo pacman -S ethtool")
    try:
        subprocess.run(["sudo", "ethtool", "-s", "enp2s0", "wol", "g"], check=True, timeout=10)
        print("[SA] Wake-on-LAN включён на enp2s0")
        subprocess.run(["sudo", "systemctl", "enable", "--now", "wakeonlan"], capture_output=True, timeout=10)
    except Exception as e:
        _die(f"Не удалось включить WoL: {e}")

def _wol_off():
    if not shutil.which("ethtool"):
        _die("ethtool не установлен. Установите: sudo pacman -S ethtool")
    try:
        subprocess.run(["sudo", "ethtool", "-s", "enp2s0", "wol", "d"], check=True, timeout=10)
        print("[SA] Wake-on-LAN выключен на enp2s0")
        subprocess.run(["sudo", "systemctl", "disable", "--now", "wakeonlan"], capture_output=True, timeout=10)
    except Exception as e:
        _die(f"Не удалось выключить WoL: {e}")

def cmd_wol():
    """Показать информацию о Wake-on-LAN. sa wol on — включить, sa wol off — выключить"""
    print("── Wake-on-LAN ────────────────────────────")
...
def cmd_proxy(args):
    """Установить прокси для Telegram API"""
    url = " ".join(args).strip().strip("\"'")
    if not url:
        _die("Укажите URL прокси: sa proxy socks5://127.0.0.1:1080")
    if not config.validate_proxy(url):
        _die("URL прокси должен начинаться с http://, https://, socks5:// или socks5h://")
    config.set_proxy(url)
    print(f"[SA] Прокси сохранён: {url[:50]}...")
    print("[SA] Перезапустите сервис: sa restart")


AGENT_SERVICE = """
[Unit]
Description=Super ASK Local Agent
...
User=%s
WorkingDirectory=%s
Environment=SUPERASK_SERVER=%s
Environment=SA_AGENT_MODE=%s
ExecStart=%s/agent/agent.py
Restart=always
RestartSec=5
...
def cmd_agent(args):
    ...
    if subcmd == "on":
        ...
        agent_mode = os.environ.get("SA_AGENT_MODE") or config.get_agent_mode()
        service_content = AGENT_SERVICE % (user, PROJECT_DIR, server_url, agent_mode, PROJECT_DIR)
        svc_path = Path("/tmp/superask-agent.service")
        svc_path.write_text(service_content)
        subprocess.run(["sudo", "cp", str(svc_path), "/etc/systemd/system/superask-agent.service"], check=True, timeout=10)
        subprocess.run(["sudo", "systemctl", "daemon-reload"], check=True, timeout=10)
        subprocess.run(["sudo", "systemctl", "enable", "--now", "superask-agent"], check=True, timeout=10)
        print("[SA] Агент запущен и добавлен в автозагрузку")
        return

    if subcmd == "off":
        subprocess.run(["sudo", "systemctl", "disable", "--now", "superask-agent"], capture_output=True, timeout=10)
        print("[SA] Агент остановлен и удалён из автозагрузки")
        return

    if subcmd == "status":
        r = subprocess.run(["systemctl", "is-active", "superask-agent"], capture_output=True, text=True, timeout=5)
        status = r.stdout.strip()
        print(f"  Агент: {'✅ активен' if status == 'active' else '❌ ' + status}")
        if server_url:
            print(f"  Сервер: {server_url}")
        r2 = subprocess.run(["systemctl", "is-enabled", "superask-agent"], capture_output=True, text=True, timeout=5)
        enabled = r2.stdout.strip()
        print(f"  Автозагрузка: {'✅' if enabled == 'enabled' else '❌'} {enabled}")
        return
    ...
    if subcmd == "logs":
        ...
        print("── Логи агента ──────────────────────────")
        print(r.stdout if r.stdout.strip() else "(логов нет)")
        print("──────────────────────────────────────────")
        return

    if subcmd == "restart":
        subprocess.run(["sudo", "systemctl", "restart", "superask-agent"], capture_output=True, timeout=10)
        time.sleep(1)
        r = subprocess.run(["systemctl", "is-active", "superask-agent"], capture_output=True, text=True, timeout=5)
        status = r.stdout.strip()
        print(f"  Агент: {'✅ активен' if status == 'active' else '❌ ' + status}")
    ...
    _die("Использование: sa agent on/off/restart/status/logs")
...
def _main():
    ...
    if cmd == "proxy" and args:
        arg = args[0].lower()
        if arg == "off":
            config.clear_proxy()
            print("[SA] Прокси отключён")
            print("[SA] Перезапустите сервис: sa restart")
            return
        else:
            cmd_proxy(args)
    ...
    if cmd == "server" and args:
        config.set_server_url(args[0])
        print(f"[SA] URL сервера сохранён: {args[0]}")
        return

    if cmd == "apikey" and args:
        config.set_api_key(args[0])
        print("[SA] API-ключ сохранён")
        return

    if cmd == "mode":
        if args:
            config.set_agent_mode(args[0])
            print(f"[SA] Режим изменён: {args[0]}")
            print("[SA] Перезапустите агента: sa agent restart")
        else:
            print(f"[SA] Текущий режим: {config.get_agent_mode()}")
            print("  proxy — бот на ПК, Render как прокси")
            print("  relay — бот на Render, агент опрашивает")
        return

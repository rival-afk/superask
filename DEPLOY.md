...
## Шаг 5. Запускаем локального агента на ПК
...
```bash
sa agent status
# Должно быть: Агент: ✅ активен
```

Теперь все команды из Telegram будут уходить на Render, а Render — передавать их агенту на ПК.
